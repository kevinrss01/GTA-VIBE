...
## The city with its mouth open, 2026-08-31

Two audio defects a player reported, and the feature they turned into.

### The cars made no noise

**They did, and that was the problem.** An ambient car was ONE looping voice
playing `veh/engine-far`, which is a recording of TYRE ROLL. The reasoning is
in `engineCurve.ts` and was explicit: ambient traffic "is always heard through
a panner at 15 m or more where the layering is inaudible".

Standing on a pavement that premise does not hold. The panner's reference
distance is 7 m and cars pass inside it constantly, so what the player actually
heard when a car went past was a broad rubber hiss under the district ambience
and the `ambience/traffic-hum` bed - which is exactly what "the cars are not
making any noise" describes. A car queueing at a red light was worse: it was
voiced as tyre noise at a standstill, a sound that does not exist.

An ambient car is now TWO layers on one panner and one filter, the same split
the player's own car uses:

| layer | asset | driven by |
| --- | --- | --- |
| engine | `veh/engine-idle` | `gearFor` - rate and level off the rev model |
| tyres | `veh/engine-far` | road speed alone, silent below 1.6 m/s |

They diverge on every gearchange, which is the point. The engine layer uses one
shared recording for the whole pool - a source cannot change its buffer and the
pool is reassigned four times a second - so the CLASS still comes from the rate
and the filter, exactly as it always did for the tyres. The saloon idle was
chosen because it is 70.8 per cent low rumble, which is the part of an engine
that survives an HRTF panner and a lowpass.

**Measured on the shipped graph**, standing on Harbour Walk with six cars in
ordinary traffic inside `TRAFFIC_RADIUS`: engine gains 0.371 to 0.458 at
playback rates 1.07 to 1.30, tyre gains 0 to 0.243. Before the change the only
layer present was the tyre one at those same 0 to 0.243, so a car at a junction
went from silent to clearly audible and a moving one roughly doubled.

`TRAFFIC_VOICES` went from 5 to 7, which is what a junction needs. Cost per
voice went from four nodes to six; the persistent set is 52 nodes with the
player driving, against a ceiling of 94 with every transient one-shot in
flight. `tests/streetAudio.test.ts` pins both.

### Nobody said anything

`CrowdVoice` is a new mixer layer for BARKS, deliberately separate from
`Dialogue`. Dialogue is a scripted line with a subtitle, played once,
unspatialised, at a moment the mission chose. A bark has no script position, is
picked from a pool, is spatialised on whoever says it, and is competing with an
engine and a siren. The two share nothing but the word "speech".

**32 lines, ten library voices**, generated by `tools/generate-crowd-voices.mjs`
with `eleven_multilingual_v2` at `mp3_44100_128`, peak-normalised towards
-6 dBFS like every other one-shot here. Three voices for the police and seven
for everybody else, and that asymmetry is the point: a pursuit that arrives in
eight different voices reads as a crowd rather than as the police, while a city
whose every pedestrian sounds the same reads as one person following you
around. `Lily` and `Callum` are deliberately absent - they are Sable and Teo,
and the street must not sound like the mission cast.

| pool | lines | when |
| --- | --- | --- |
| `vox/pol-stop-*`, `vox/pol-halt-*` | 6 | an officer on foot, inside `OFFICER_VOICE_RANGE`, who can see the player |
| `vox/pol-pullover-*` | 2 | a unit's PA, inside `UNIT_VOICE_RANGE`, while the player is driving |
| `vox/pol-radio-*` | 3 | the frame a pursuit opens |
| `vox/pol-lost-*` | 2 | the frame the last unit loses sight of the player |
| `vox/chat-*` | 14 | a conversation group hands the floor over inside `CHATTER_RADIUS` |
| `vox/react-gun-*` | 3 | a gunshot the crowd hears |
| `vox/react-shove-*` | 2 | somebody knocked down by a car |

A car tells you to pull over and a man on foot tells you to get down, and the
two come from different places for a structural reason: a unit only sets
...
reactions) ON TOP of the per-officer and per-group ones the producers already
apply; no immediate repeats from a pool; and chatter dropped outright indoors
and beyond 20 m, because overhearing two strangers through a wall from across a
junction is a bug rather than a detail.

The lines are **deferred out of the boot preload** (`LAZY_ASSET_IDS`) and asked
for by `CrowdVoice.preload()` on the start gesture: 1.3 MB that nobody can hear
in the first second of a session.

### Conversations

The chatter needs somebody to be talking, so `crowd.ts` grew conversation
groups: two people who have both stopped near each other are paired, turned to
face one another, and hand a notional floor back and forth for up to 17
seconds. It is built ON TOP of the pause that already existed - a conversation
is a pause with a partner and an owner for its timer - so it is out of the
movement code entirely.

**They are formed near the listener on purpose.** The crowd covers 63,000
square metres and a conversation carries 20 m, which covers 900: spending a
global budget across the whole crowd put an expected 0.07 groups inside earshot
and the feature simply never happened. Forming them inside `CHAT_FORM_RADIUS`
concentrates the same budget where it is experienced, at no cost in
plausibility, because a conversation 120 m away is four pixels tall and silent
either way.

A walker may be recruited from 2.4 m and somebody already stopped from 3.2 m.
Requiring two independent pauses to land within a couple of metres is a
four-per-cent event and gave about one group alive at a time; being hailed by
somebody at arm's length reads as two people who know each other.

**The invariant this could have broken.** `watchStall` polices `walk` and
`cross` only - standing still is a decision, not a failure - so nothing else in
the crowd would ever have moved two people who were talking, and
`tests/crowdCorners.test.ts` holds the whole crowd to nobody standing still for
thirty seconds. Hence the 17 second ceiling, the 30 second cooldown before
either party may start another, and the release path that always runs.
